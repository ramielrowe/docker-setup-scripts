...
function get_container_variables {
    ...
    ICC_HOSTPORT_TEMPLATE='{{ "com.apmelton.webhead.icc.hostport" }} {{.NetworkSettings.Networks.'"${OVERLAY_NETWORK}"'.IPAddress}}'
    echo "$(docker inspect -f "${LABELS_TEMPLATE}${ICC_HOSTPORT_TEMPLATE}" ${1})"
}
...
for ID in $PUBLISH_IDS; do
    ...
    if [ "${IS_ICC}" == "true" ]; then
       ICC_HOSTPORT=$(get_container_variable 'com.apmelton.webhead.icc.hostport' "${CONTAINER_VARS}")
       DOMAIN_MAPPINGS="${DOMAIN_MAPPINGS} ${CUR_DOMAIN}=${ICC_HOSTPORT}"
    else
